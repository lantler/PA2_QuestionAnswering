...
def gen_where_response(user_input, wikipedia_summary):
    ...
    sent_with_loc = find_sentence_with_location(wikipedia_summary)
    if sent_with_loc != None:
        last_loc = find_last_gpe_in_sentence(sent_with_loc)
        if last_loc != None:
            return object_name.title() + " was in " + last_loc + "."
